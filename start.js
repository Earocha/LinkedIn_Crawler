...
import { Info,Credentials } from './functions/helper'
...
const Start = () =>{
    (async()=>{
        ...
        var countLinked = 1
        for(var i in profileFromPodio){
            if(profileFromPodio[i].LinkedIn_Profile === "" && profileFromPodio[i].Email !== "" ){

                //console.log("SearchByEmail");// && profileFromPodio[i].Email === "" 

            }else if (profileFromPodio[i].LinkedIn_Profile !== "") {
                try{
                    ...
                    let newpage = await page.goto(profileFromPodio[i].LinkedIn_Profile,{
                    ...
                    profileData.push({
                        "item_id":profileFromPodio[i].Item_Id,
                        "Name":name,
                        "JobTitle":job_title,
                        "About":about,
                    ...
                    await page.close(); 
                    await browser.close();
                    console.log("THE ID CHECK HERE",profileFromPodio[i].Item_Id)
                    console.log("Inserting Data");
                    await Delay()
                    var checkdup = await con.execute("SELECT * FROM linked WHERE item_id = "+"'"+`${profileFromPodio[i].Item_Id}`+"'")
                    console.log("length sql result =>",checkdup[0].length);
                    var DataInsert = [[
                        profileFromPodio[i].Item_Id,
                        name,
                        job_title,
                        about,
                        current_employer,
                        ...
                        skills,
                        recommendation,
                        profileFromPodio[i].LinkedIn_Profile
                    ]]
                    if(checkdup[0].length === 0){
                        var scripteInsert = "INSERT INTO linked(item_id, name, job_title, about, current_employeer, experience,education,skills,recommendation,link) VALUES ? ";
                    ...
                    skills = "";
                    recommendation = "";
                    console.log(profileFromPodio.length);
                    await Delay()
                    countLinked +=1
            ...
            }else if (profileFromPodio[i].LinkedIn_Profile !== ""  && profileFromPodio[i].Email !== "" ){
